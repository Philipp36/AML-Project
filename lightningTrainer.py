...
class trainerLightning(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        bin_vec = truth != 3
        loss1 = self.lossBoxes(heat_pred.double()[bin_vec], heat_resized.double()[bin_vec])
        loss2 = self.lossLabels(label_pred, truth)
        self.log("HeatMapLoss/Train", loss1, on_step=True, on_epoch=True, logger=True)    # TODO: switch to mean over epoch trough on_epoch=True?
        self.log("LabelLoss/Train", loss2, on_step=True, on_epoch=True, logger=True)      # TODO: switch to mean over epoch
        LOSS = loss1 + loss2
        return LOSS
